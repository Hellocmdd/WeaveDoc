...
find_chat_model() {
    ...
    if [[ -f "$MODELS_DIR/Qwen3.5-4B-Q4_K_M.gguf" ]]; then
        echo "$MODELS_DIR/Qwen3.5-4B-Q4_K_M.gguf"
        return 0
    fi

    local first_model=""
    while IFS= read -r candidate; do
        if [[ "$candidate" != "$DEFAULT_EMBED_MODEL" && "$candidate" != "$DEFAULT_RERANKER_MODEL" ]]; then
...
wait_for_server() {
    ...
    echo "[run_weavedoc] $name did not become healthy in time" >&2
    echo "[run_weavedoc] check log: $log_path" >&2
    return 1
}
...
start_server_if_needed() {
    ...
    "$LLAMA_SERVER_BIN" \
        ...
        --gpu-layers "$LLAMA_SERVER_GPU_LAYERS" \
