...
- `RAG_PIPELINE_MODE = refactored`（运行模式在 `LocalAiService` 中硬编码为 `refactored`；`RagOptions` 不暴露此环境变量 — 服务始终使用 `refactored` 模式）
...
- `RAG_CHAT_PROVIDER = llama_server`（设为 `deepseek` 使用云端；另支持 `CLOUD_API_KEY` / `CLOUD_MODEL` / `CLOUD_BASE_URL` / `CLOUD_ENABLE_THINKING` / `CLOUD_REASONING_EFFORT`，`DEEPSEEK_*` 为后备别名）
...
- 管线模式在 `LocalAiService` 中硬编码为 `refactored`；`RagOptions` 不暴露 `RAG_PIPELINE_MODE` 环境变量，用户无法通过环境变量切换运行模式，现有文档中的环境变量说明可能引发误解
