...
""";
        ...
        IReadOnlyList<string> targetFilePaths)
    {
        ...
        {
            builder.AppendLine("流程综合指令: 流程类问题的答案通常分散在多个上下文块中，很少有单个块直接完整描述整个流程。你必须综合所有相关上下文块，直接陈述流程中系统实际使用的算法、组件、参数和结果，不要用\"可以按...链路来理解\"等元描述开头。不要因为单个块信息不完整就回答'未覆盖'。");
        }
        if (queryProfile.IsFollowUpExpansion)
        {
            builder.AppendLine("追问延续指令: 当前问题是对上一轮主题的补充展开。回答时必须延续最近对话里已经建立的主题对象与主链路。展开时应围绕该主题的核心机制深入，在原有主链路上补足关键步骤、输入输出、执行方式或结果；不要转移到辅助话题或改答成其他相关但不同的话题。");
        }
