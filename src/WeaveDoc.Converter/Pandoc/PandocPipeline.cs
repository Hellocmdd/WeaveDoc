...
public class PandocPipeline
{
    ...
    private static readonly string[] PandocNames = OperatingSystem.IsWindows()
        ? ["pandoc.exe"]
        : ["pandoc", "pandoc.exe"];

    private static readonly string[] PandocRelativePaths = OperatingSystem.IsWindows()
        ? ["tools/pandoc/pandoc.exe"]
        : ["tools/pandoc/bin/pandoc", "tools/pandoc/pandoc", "tools/pandoc/pandoc.exe"];

    /// <param name="pandocPath">Pandoc 可执行文件路径，默认依次查找 tools/pandoc/、系统 PATH</param>
    ...
    private static string ResolvePandocPath()
    {
        ...
        foreach (var rel in PandocRelativePaths)
        {
            var localPath = Path.Combine(AppContext.BaseDirectory, rel.Replace('/', Path.DirectorySeparatorChar));
        ...
        {
            foreach (var name in PandocNames)
            {
