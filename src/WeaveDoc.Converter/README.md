...
Markdown ──→ [MarkdownMathNormalizer] ──→ [Pandoc + LuaFilters + reference.docx] ──→ raw.docx
                                             ...
                                             [OpenXmlStyleCorrector]
                                               ...
                                               └── ApplyHeaderFooter        (页眉页脚)
                                                        │
                                                        ↓
                                                 修正后 DOCX ──→ 输出或由 Syncfusion DocIO 生成 PDF
```
...
│   ├── SyncfusionPdfConverter.cs     #   Syncfusion DocIO DOCX → PDF
...
**SyncfusionPdfConverter** — Syncfusion DocIO 封装：

- `ConvertToPdf` — DOCX → PDF，保留所有 OpenXML 样式（背景、边框、页面设置、页眉页脚）
...
5. 若输出 PDF，调用 SyncfusionPdfConverter.ConvertToPdf
