...
> **当前状态**：测试通过，详见 [Converter 测试 README](../../tests/WeaveDoc.Converter.Tests/README.md)，0 构建错误
...
│   ├── MarkdownMathNormalizer.cs     #   Markdown 公式预处理
│   ├── IPdfConverter.cs              #   PDF 转换器统一接口
│   ├── CompositePdfConverter.cs      #   PDF 引擎优先级选择：Word → LibreOffice → Syncfusion
│   ├── PdfRendererDetector.cs        #   本机 PDF 引擎检测
│   ├── WordComPdfConverter.cs        #   Microsoft Word COM DOCX → PDF
│   ├── LibreOfficePdfConverter.cs    #   LibreOffice headless DOCX → PDF
│   ├── SyncfusionPdfConverter.cs     #   Syncfusion DocIO DOCX → PDF 兜底
...
- Markdown 公式规范化：兼容 `$ ... $` 内侧带空格的文本，转为 Pandoc 可识别的 dollar math；跳过代码块和行内代码
- Lua Filters 自动发现：运行时扫描 `LuaFilters/*.lua`，自动附加到 Pandoc 命令

**PDF 转换器链** — 通过 `IPdfConverter` 统一 DOCX → PDF 行为：

- `CompositePdfConverter` — 按 `Microsoft Word → LibreOffice → Syncfusion DocIO` 顺序尝试
- `PdfRendererDetector` — 检测 Word COM、LibreOffice `soffice` 和 Syncfusion 兜底能力
- `WordComPdfConverter` — Windows 桌面环境下调用 Word `ExportAsFixedFormat`，优先保证中文字体保真
- `LibreOfficePdfConverter` — 调用本机 `soffice --headless --convert-to pdf`
- `SyncfusionPdfConverter` — 无外部高保真引擎时兜底，保留当前字体替代与 fallback 逻辑
...
3. PandocPipeline 规范化 Markdown 公式并执行 Markdown → Docx 转换
4. OpenXmlStyleCorrector 精修样式（写入样式定义 → 清除冗余内联 → 页面设置 → 页眉页脚）
5. 若输出 PDF，通过 CompositePdfConverter 按 Word → LibreOffice → Syncfusion 顺序导出

---

## 构建与外部依赖

Pandoc 无需手动安装。通过 MSBuild Target（`tools/DownloadExternalTools.targets`）在 `BeforeBuild` 阶段自动检测：

- 若 `tools/pandoc/` 目录不存在，自动执行 `tools/setup-tools.ps1` 下载
- 下载的工具二进制通过 `Content` 配置复制到输出目录

```bash
dotnet build src/WeaveDoc.Converter/WeaveDoc.Converter.csproj   # 自动下载依赖
dotnet test tests/WeaveDoc.Converter.Tests/
```

PDF 高保真导出依赖用户本机已有渲染器：

- Windows 上优先使用 Microsoft Word COM（`Word.Application`）
- 未检测到 Word 时，尝试 LibreOffice `soffice`
- 两者都不可用时，回退 Syncfusion DocIO；此模式可能降低中文字体保真度
...
核心库测试覆盖 AFD 解析、样式映射、Pandoc 管道、配置管理、BibTeX 解析、公式预处理和错误信息格式化。

端到端测试覆盖完整链路：`Parse → ReferenceDoc → Pandoc → StyleCorrector → PageSettings → HeaderFooter`，验证样式定义（非内联）包含正确的字体/字号属性。

详细测试数量与用例清单见 [Converter 测试 README](../../tests/WeaveDoc.Converter.Tests/README.md)。
