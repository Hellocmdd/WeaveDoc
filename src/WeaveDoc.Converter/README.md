...
- Windows 继续使用 `tools/setup-tools.ps1`
- Linux/macOS 使用 `tools/setup-tools.sh`
- 下载后的产物统一落在 `tools/pandoc/`
- 若当前环境无法获得可用 Pandoc，管道会退回仓库内置的 fallback 路径用于当前测试/构建环境
- 可通过 `SkipExternalToolsDownload=true` 跳过自动准备
...
| `PandocPipelineTests` | 21 | 管道转换、样式修正、页眉页脚、3 模板端到端 |
