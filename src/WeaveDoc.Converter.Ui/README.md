...
┌─────────────────────────────────────────────────────┐
│                    MainWindow                        │
│              "WeaveDoc 转换工具" 800×500               │
│              TabControl 双标签页布局                   │
├────────────────────┬────────────────────────────────┤
│    TemplateTab     │         ConvertTab              │
│    模板管理         │         文档转换向导              │
├────────────────────┼────────────────────────────────┤
│  DataGrid 模板列表  │  Step 1: 选择 Markdown 文件     │
│  刷新 / 种子 / 导入 │  Step 2: 选择模板 (ComboBox)    │
│  删除操作           │  Step 3: 导出格式 (Docx/PDF)    │
│  状态栏（模板总数）  │  输出目录 + 转换按钮 + 日志      │
└────────────────────┴────────────────────────────────┘
          │                         │
          ▼                         ▼
...
- **操作按钮**：
  ...
  - 刷新 — 重新加载模板列表
  - 种子模板 — 调用 `EnsureSeedTemplatesAsync` 发现内置模板
  - 导入 — 从外部 JSON 文件导入模板
  - 删除 — 移除选中模板
...
- **输出目录** — 文件夹浏览对话框
- **转换按钮** — 调用 `DocumentConversionEngine.ConvertAsync()`
- **状态标签** — 实时显示转换状态
- **日志框** — 显示转换过程详细日志
